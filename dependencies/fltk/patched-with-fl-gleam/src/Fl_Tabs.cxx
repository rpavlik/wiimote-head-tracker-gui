...
void Fl_Tabs::draw() {
  ...
  if (damage() & FL_DAMAGE_ALL) { // redraw the entire thing:
	...
	if (selection_color() != c) {
	  if (!(Fl::scheme_ && !strcasecmp(Fl::scheme_, "xp"))) {
        ...
        draw_box(box(), x(), y()+(H>=0?H:0), w(), h()-(H>=0?H:-H), selection_color());
...
void Fl_Tabs::draw_tab(int x1, int x2, int W, int H, Fl_Widget* o, int what) {
  ...
  if (H >= 0) {
    ...
    if (Fl::scheme_ && !strcasecmp(Fl::scheme_, "xp")) {
      draw_box(bt, x1, y() + yofs, W, H + 10 - yofs, color());

      if (sel) {
        int dx = Fl::box_dx(box());
        int dw = Fl::box_dw(box());
        fl_rectf(x1 + dx, y() + dy, W - dw, 3, 255, 199, 57);
      }
